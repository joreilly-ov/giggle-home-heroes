...
function logEvent(event: Record<string, unknown>) {
  try {
    console.log(JSON.stringify({ fn: 'analyse-photos', ts: new Date().toISOString(), ...event }));
  } catch {
    console.log('analyse-photos log serialise failed');
  }
}
...
serve(async (req) => {
  ...
  const limit = checkRateLimit(`analyse-photos:${user.id}`, 10, 60_000);
  if (!limit.allowed) {
    logEvent({
